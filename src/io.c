 ...
 * directly. Instead call |io_state_send| with the PDU write buffer to
 * append the PDU to the send queue. |io_state_send| will add the file
 * descriptor to the epoll loop; waiting for writeability. Directly
 * sending a PDU could block the epoll loop, which is to be avoided.
 *
...
create_sockaddr_un(const char* socket_name, struct sockaddr_un* addr)
{
  ...
  pathsiz = NAME_OFFSET + siz; /* include leading '\0' */
