...
/* |deg2rad| takes an angle in degrees and returns its value in radiant. */
static float
deg2radf(float deg)
{
 ...
 * Sensors lists
 *
 * Sensorsd maintains a list of known sensors. This is required to
 * work around broken systems that do not include the sensor's type
 * in the generated events. This bug can be observed on emulator
 * targets.
 *
 ...
 * supposed to free the list by a call to |free_sensors_list|. To search
 * for a sensor by id, call |find_sensor_by_id|. It takes a sensors
 * list and the id and returns the sensor's entry in the list. The value
 * of |id| must be less than SENSORS_HANDLE_COUNT. If the sensor doesn't
 * exist, the returned entry's field |handle| will by 0.
 */
 ...
 * Protocol helpers
 *
 * These helper functions append Sensors-specific data to a PDU. They
 ...
 * releases. The helpers try hary hard to always send out meaning full
 * values. So when you port sensorsd to a new Android release, please
 * ensure that these functions are still up to date. Differences should
 * be handled here instead of changing the IPC protocol.
 *
...
append_sensors_t(struct pdu* pdu, const struct sensor_t* sensor)
{
  ...
  int32_t max_delay = sensor->maxDelay;
  uint32_t flags = sensor->flags;
#else
  int32_t max_delay = INT32_MAX;
  uint32_t flags = g_default_sensor_flags[sensor->type];
#endif
 ...
 * implemented for detected sensors and sensor events and errors.
 *
 * The IPC protocol also defines a notification for removing sensors, but
 * that's currently not implemented. The Android Sensors API doesn't support
 * hotplugging, so sensors never get removed.
 *
 * The function |sensor_detected_ntf| creates a corresponding notification
 ...
 * and special cases to deal with different versions of Android. Each of these
 * functions expects to run on the poll thread.
 *
 ...
 * are not guaranteed to succeed. If we observe a failure when sending them, we
 * abort the daemon immediately. This prevents error cascades and free's up
 * resources for other components. |error_ntf| takes the error code as parameter,
 * although only ERROR_NOMEM is currently used.
 */
 ...
 *
 * All command handlers work the same, the receive the PDU that has been
 * sent by the client. Picking the correct handler for each service and
 * opcode is implemented by the generic protocol code.
 *
 * The command handler decodes and validates the arguments in the PDU and
 * an dexecutes the operation. Usually this means calling a funtion of the
 * Android driver.
 *
 * Whatever result the operation returns, is packed into a response PDU
 * and sent to the client. Errors are returned from the handler and packed
 * by the protocol framework.
 */
...
enable_sensor(const struct pdu* cmd)
{
  ...
  if (res < 0) {
    ALOGE_ERRNO_NO("sensors_poll_device_t::activate", -res);
...
set_period(const struct pdu* cmd)
{
  ...
  if (res < 0) {
    ALOGE_ERRNO_NO("sensors_poll_device_t::setDelay", -res);
 ...
 * function's implementation has details on that.
 *
...
{
  /* THE ONLY SAFE SENSORS FUNCTION TO RUN ON THE POLL THREAD IS
   * |sensors_poll_device_t::poll|. All other functions must run
   * concurrently on the I/O thread. We hand-over the list of
   * sensors to the I/O thread, so we don't to detect them here.
   */
  ...
  while (1) {
    ...
    if (res < 0) {
      if (res == -ENOMEM) {
        ...
        ALOGE_ERRNO_NO("sensors_poll_device_t::poll", -res);
        break;
      }
    }
    count = res;
...
(*register_poll(void (*send_pdu_cb)(struct pdu_wbuf*)))(const struct pdu*)
{
  ...
  if (!(g_sensors_module && g_poll_device && g_poll_thread)) {
    ...
    if (err) {
      ALOGE_ERRNO_NO("sensors_open", err);
      return NULL;
    }
...
err_create_sensors_list:
  ...
  if (err) {
    ALOGW_ERRNO_NO("sensors_close", err);
  }
  g_poll_device = NULL;
